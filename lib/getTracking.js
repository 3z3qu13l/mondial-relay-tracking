...
module.exports = async (trackingId, postalCode) => {
    ...
    const lastUpdate = lastStep.datetime;
    const currentStatus = lastStep.text;
    const isComplete = lastStep.text === constants.FINAL_TEXT;
    if (lastStep.location) trackingInfos.location = lastStep.location;

    const firstStep = history[history.length - 1] || {};
    const startDate = firstStep.datetime;

    const duration = Math.ceil((lastStep.datetime - firstStep.datetime) / constants.MILLISEC_IN_DAYS);
